...
シーン内でアニメーションが切り替わるたびに、アンカー、参加者 bounds、有効領域、プリセット候補を再評価する。高い位置のアニメーションから低い位置のアニメーションへ移行した際に、以前の高いカメラ位置をそのまま維持しないことを要件候補とする。P+ からアニメーション変更を検出する具体的なイベント境界は未調査である。
...
- アニメーション変更後、node transform の反映を確認してアンカーを計算するタイミング。
