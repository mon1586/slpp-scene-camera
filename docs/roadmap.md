...
- 安定したpreset IDを使い、一覧取得、作成、更新、削除を行うrepositoryを実装する。
- 保存時は一時ファイルからの置換などを使い、書き込み途中のファイルを正常データとして残さない。
- 作成・更新時に数値の有限性、値域、必須項目を検証する。
- reload、同名または同IDの扱い、削除中の選択プリセット、schema migrationの範囲を決める。
- まずstorageと操作APIをunit testで固定し、ゲーム内UIは同じAPIを呼ぶ薄い層として追加する。
- この段階ではLOS・clearanceを保存可否へ影響させない。

ゲーム内編集では、現在のカメラをアンカー相対poseへ逆変換して作成・更新する。採用するUI frameworkと操作方法はここで確定するが、clearance表示と保存時警告は後段で追加する。
